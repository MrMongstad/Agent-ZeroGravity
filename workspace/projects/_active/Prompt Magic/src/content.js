...
// STEP 4.5: The Anti-Fragile Fallback Engine
function findTargetInput() {
  if (platformConfig && platformConfig.input) {
    const el = document.querySelector(platformConfig.input);
    if (el) return el;
  }

  // Fallback: Find the largest textarea or contenteditable on the page
  ...
  for (const el of candidates) {
    ...
    // Ignore hidden or tiny inputs (e.g., search bars)
    if (area > maxArea && rect.width > 50 && rect.height > 20) {
      maxArea = area;
      largestEl = el;
    }
  }
...
async function loadSelectors() {
  try {
    ...
    console.warn("Prompt Magic: Failed to load selectors.json, relying on fallback engine.", error);
    platformConfig = null;
  } finally {
    initGhostUI(); // Always initialize, the fallback engine will handle missing configs
  }
}
